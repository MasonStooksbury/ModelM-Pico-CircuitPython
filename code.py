...
for k, v in key_map.items():
    for k2, v2 in v.items():
        if 'SHIFT' in v2['keyname'] or 'ALT' in v2['keyname'] or 'CONTROL' in v2['keyname']:
            modifier_dict[v2['keyname']] = {
                'col': k, 'row': k2, 'keycode': v2['keycode']}
...
loop = True
while loop:
    is_shift, is_alt, is_ctrl = checkAndPressModifiers(
        is_shift, is_alt, is_ctrl)
    for col in cols:
        col['pin'].value = True
        for row in rows:
            ...
            if not is_modifier and (pin['last_actual_state'] == 0) and (pin['actual_state'] == 1):
                ...
                if pin['start_hold_time'] > 0 and (current_time - pin['start_hold_time']) > holdTimeCheck:
                    ...
                    kbd.press(int(key_map[col['name']]
                              [row['name']]['keycode']))
